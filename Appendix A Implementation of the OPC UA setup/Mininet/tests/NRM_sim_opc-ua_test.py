...
# #TODO: Change talker and listener cmd to OPC UA, also set up other hosts
    ...
    talker_pub_cmd      = './../../OPC-UA\ PubSub/publisher/tutorial_pubsub_mqtt_publish --url opc.mqtt://10.1.1.2:1883 --topic topic1 topic2 --freq 600 500'
    talker_broker_cmd = 'mosquitto -c ../../mosquitto/virtual_host.conf'

    listener_sub_cmd = './../../OPC-UA\ PubSub/subscriber/tutorial_pubsub_mqtt_subscriber --url opc.mqtt://10.1.1.2:1883 --topic topic1 topic2 --freq 600 500'
    listener_broker_cmd = 'mosquitto -c ../../mosquitto/proxy.conf'
    #five_tuple_cmd = 'python3 ../../SEAL/seal/five_tuple_sub.py'
    five_tuple_cmd = 'python3 ../../SEAL/seal/five_tuple_log.py'
...
#TODO: Start NRM server simulation
  ...
  #Start OPC UA related stuff
    info("\nStarting virtual host broker: '%s'\n"%listener_broker_cmd)
    virtual_host_broker_process = net['cloud_b'].popen(run_in_terminal(net['cloud_b'].pid, listener_broker_cmd), shell=True)
    

    info("\nStarting '%s'\n"%seal_user)
    topic_watcher_process = net['cloud_b'].popen(run_in_terminal(net['cloud_b'].pid, listener_broker_cmd), shell=True)
    info("\nStarting '%s'\n"%seal_user)
    fivetuple_finding_process = net['cloud_b'].popen(run_in_terminal(net['cloud_b'].pid, five_tuple_cmd), shell=True)
    

    info("\nStarting proxy broker: '%s'\n"%talker_broker_cmd)
    proxy_broker_process = net['publisher'].popen(run_in_terminal(net['publisher'].pid, talker_broker_cmd), shell=True)
    info("\nStarting '%s'\n"%bandwidth_cmd)
    unique_flow_stats_process = net['publisher'].popen(run_in_terminal(net['publisher'].pid, bandwidth_cmd), shell=True)
    info("\nStarting '%s'\n"%talker_pub_cmd)
    opc_ua_publisher = net['publisher'].popen(run_in_terminal(net['publisher'].pid, talker_pub_cmd), shell=True)

    info("\nStarting '%s'\n"%listener_sub_cmd)
    opc_ua_subscriber = net['publisher'].popen(run_in_terminal(net['publisher'].pid, listener_sub_cmd), shell=True)
...
#     mininet_controll_process = net['connexion'].popen(run_in_user_terminal(mininet_controll), shell=True)
    ...
    try:
        ...
        try:
            opc_ua_publisher.terminate()
            opc_ua_publisher.wait(timeout=5)
        except opc_ua_publisher.TimeoutExpired:
            print(f"{opc_ua_publisher} didn’t terminate in time. Killing...")
            try:
                opc_ua_publisher.wait(timeout=5)
            except opc_ua_publisher.TimeoutExpired:
                print(f"{opc_ua_publisher} didn’t terminate in time. Killing...")
                opc_ua_publisher.kill()
        
        try:
            opc_ua_subscriber.terminate()
            opc_ua_subscriber.wait(timeout=5)
        except opc_ua_subscriber.TimeoutExpired:
            print(f"{opc_ua_subscriber} didn’t terminate in time. Killing...")
            try:
                opc_ua_subscriber.wait(timeout=5)
            except opc_ua_subscriber.TimeoutExpired:
                print(f"{opc_ua_subscriber} didn’t terminate in time. Killing...")
                opc_ua_subscriber.kill()
        
        try:
            proxy_broker_process.terminate()
            proxy_broker_process.wait(timeout=5)
        except proxy_broker_process.TimeoutExpired:
            print(f"{proxy_broker_process} didn’t terminate in time. Killing...")
            try:
                proxy_broker_process.wait(timeout=5)
            except proxy_broker_process.TimeoutExpired:
                print(f"{proxy_broker_process} didn’t terminate in time. Killing...")
                proxy_broker_process.kill()
        
        try:
            fivetuple_finding_process.terminate()
            fivetuple_finding_process.wait(timeout=5)
        except fivetuple_finding_process.TimeoutExpired:
            print(f"{fivetuple_finding_process} didn’t terminate in time. Killing...")
            try:
                fivetuple_finding_process.wait(timeout=5)
            except fivetuple_finding_process.TimeoutExpired:
                print(f"{fivetuple_finding_process} didn’t terminate in time. Killing...")
                fivetuple_finding_process.kill()

        try:
            unique_flow_stats_process.terminate()
            unique_flow_stats_process.wait(timeout=5)
        except unique_flow_stats_process.TimeoutExpired:
            print(f"{unique_flow_stats_process} didn’t terminate in time. Killing...")
            try:
                unique_flow_stats_process.wait(timeout=5)
            except unique_flow_stats_process.TimeoutExpired:
                print(f"{unique_flow_stats_process} didn’t terminate in time. Killing...")
                unique_flow_stats_process.kill()
        
        try:
            topic_watcher_process.terminate()
            topic_watcher_process.wait(timeout=5)
        except topic_watcher_process.TimeoutExpired:
        ...
        try:
            ...
            virtual_host_broker_process.wait(timeout=5)
        except virtual_host_broker_process.TimeoutExpired:
            print(f"{virtual_host_broker_process} didn’t terminate in time. Killing...")
            try:
                virtual_host_broker_process.wait(timeout=5)
            except virtual_host_broker_process.TimeoutExpired:
                print(f"{virtual_host_broker_process} didn’t terminate in time. Killing...")
                virtual_host_broker_process.kill()        
