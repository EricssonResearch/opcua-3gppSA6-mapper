...
class CustomTopo (Topo):
    def build(self, **_opts):
        ...
        #TODO: Set default route for routing and parse IP, MAC and CLS of hosts
            info( '*** Add hosts\n')
            talker_host = self.addHost('publisher', cls=VLANHost, ip='10.1.1.2/24', mac='BB:00:00:00:00:10' ,defaultRoute='via 10.1.1.1', vlan=10) 
            cloud_listener_host = self.addHost('cloud_b', cls=VLANHost, ip='10.1.2.2/24', mac='CC:00:00:00:00:20', defaultRoute='via 10.1.2.1', vlan=20 ) 
            subscriber_host = self.addHost('subscriber', cls=VLANHost, ip='10.1.3.2/24', mac='DD:00:00:00:00:30', defaultRoute='via 10.1.3.1', vlan = 30 ) 
            capif_host = self.addHost('capif', cls=VLANHost, ip='10.1.4.2/24', mac='EE:00:00:00:00:040', defaultRoute='via 10.1.4.1', vlan = 40 ) 
            NR_server_sim_host = self.addHost('connexion', cls=VLANHost, ip='10.1.2.50/24', mac='CC:00:00:00:00:50', defaultRoute='via 10.1.2.1', vlan = 50 ) 
        ...
        #TODO tc-flow
            links["L1"] = self.addLink(s1, r1, intfName2='r1-eth1', cls=TCLink, **wifi_qos, params2={'ip' : '10.1.1.1/24'}) #Link between Router and Switch 1
            links["L2"] = self.addLink(s2, r1, intfName2='r1-eth2', cls=TCLink, **wifi_qos, params2={'ip' : '10.1.2.1/24'}) #Link between Router and Switch 2
            links["L3"] = self.addLink(s3, r1, intfName2='r1-eth3', cls=TCLink, **wifi_qos, params2={'ip' : '10.1.3.1/24'}) #Link between Router and Switch 3
            links["L4"] = self.addLink(s4, r1, intfName2='r1-eth4', cls=TCLink, **wifi_qos, params2={'ip' : '10.1.4.1/24'}) #Link between Router and Switch 4
            links["L5"] = self.addLink(s1, talker_host,             intfName2='s1-eth1', cls=TCLink , **wifi_qos) #OPC UA publisher
            links["L6"] = self.addLink(s2, cloud_listener_host,     intfName2='s2-eth1', cls=TCLink , **wifi_qos) #Cloud MQTT broker
            links["L7"] = self.addLink(s3, subscriber_host,         intfName2='s3-eth1', cls=TCLink , **wifi_qos) #OPC UA subscriber
            links["L8"] = self.addLink(s4, capif_host,              intfName2='s4-eth1', cls=TCLink , **wifi_qos) #CAPIF
            links["L9"] = self.addLink(s2, NR_server_sim_host,      intfName2='s2-eth2', cls=TCLink , **wifi_qos) #NR server simulation
...
# #TODO: Change talker and listener cmd to OPC UA, also set up other hosts
    ...
    NR_server_sim_cmd = 'python3 ../../Connexion/Connexion.py'
# #For testing
    seal_user = 'python3 ../../seal/seal/seal_user_NRM_tester.py'
    mininet_controll = "python3 ../../Connexion/mininet_unix_server_sim2.py"
            ...
            if data is None:
                ...
                if len(command) == 3 or len(command) == 4:
                    ...
                    #GroupManagement / Change Vlan
                        ...
                        vlan_new = int(command[2]) if command[2].isdigit() else None
                        if vlan_new is not None and command[0] in links.keys():
                            # custom_wifi_qos = dict(wifi_qos)
                            #TODO here set VLAN of of VLANHOST
                            print(vlan_new)
                            print(sock,address, "Setting vlan to %d\n" % (vlan_new))
                            links[command[0]].intf1.config(vlan=vlan_new)
                            links[command[0]].intf2.config(vlan=vlan_new)
                            # links[command[0]].intf1.config(**custom_wifi_qos)
                            # links[command[0]].intf2.config(**custom_wifi_qos)
                    ...
                    if command[1] == "delay":
                        new_delay = int(command[2]) if command[2].isdigit() else None
                        if new_delay is None:
                            new_delay = float(command[2]) if isfloat(command[2]) else None

                        if new_delay is not None and command[0] in links.keys():
                            custom_wifi_qos = dict(wifi_qos)
                            try:
                                custom_wifi_qos['delay'] = str(float(new_delay)+"ms")
                            except:
                                custom_wifi_qos['delay'] = str(int(new_delay)+"ms")

                            print(sock,address, "Setting delay to %f\n%s\n" % (new_delay, custom_wifi_qos['delay']))
                            links[command[0]].intf1.config(delay=new_delay)
                            links[command[0]].intf2.config(delay=new_delay)
                            # links[command[0]].intf1.config(**custom_wifi_qos)
                            # links[command[0]].intf2.config(**custom_wifi_qos)
                            print(custom_wifi_qos)
