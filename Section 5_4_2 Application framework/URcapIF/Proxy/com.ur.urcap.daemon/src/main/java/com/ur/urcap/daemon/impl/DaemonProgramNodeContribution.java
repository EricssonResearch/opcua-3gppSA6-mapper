...
public class DaemonProgramNodeContribution implements ProgramNodeContribution {
	private static final String NAME = "name";

	private final ProgramAPIProvider apiProvider;
	private final DaemonProgramNodeView view;
	private final DataModel model;
	private final XmlRpcDaemonInterface daemonStatusMonitor;
	private final KeyboardInputFactory keyboardInputFactory;

	public DaemonProgramNodeContribution(ProgramAPIProvider apiProvider,
										   DaemonProgramNodeView view,
										   DataModel model) {
	...
	private void setTitleAndMessage(final String title, final String message) {
		EventQueue.invokeLater(new Runnable() {
			@Override
			public void run() {
				DaemonProgramNodeContribution.this.updatePreview(title, message);
			}
		});
	}
	...
	public void generateScript(ScriptWriter writer) {
		...
		writer.assign("daemon_message", getInstallation().getXMLRPCVariable() + ".get_message(\"" + getName() + "\")");
		writer.assign("daemon_title", getInstallation().getXMLRPCVariable() + ".get_title()");
		writer.appendLine("popup(daemon_message, daemon_title, False, False, blocking=True)");
	...
	private DaemonInstallationNodeContribution getInstallation() {
		return apiProvider.getProgramAPI().getInstallationNode(DaemonInstallationNodeContribution.class);
	}
}
